...
export default function Header() {
	...
	const onHoverStartHandler = () => {
		setIsHover(true);
	};

	const onHoverEndHandler = () => {
		setIsHover(false);
	};
					...
					{headerMenu.map((item) => (item.key === 'services' ? (
						...
						>
							...
							<div
								...
								className={`absolute left-1/2 z-10 mt-[-20px] flex w-screen max-w-max -translate-x-1/2 px-4 pt-[40px] ${isHover ? '' : 'hidden'}`}
							>
