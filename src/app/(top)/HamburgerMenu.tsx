						...
						<div className="flex space-x-[160px]">
							<div className="flex flex-col">
								{leftMenuList.map((item) => {
									if (item.key === 'services') {
										return (
											...
											<div key={item.key} className="mb-[40px]">
												<Link
													href={item.path}
													className="flex flex-col text-af-md"
												>
													{item.text}
												</Link>
												{serviceDetailMenu.map((servicedetailitem) => (
													<div key={servicedetailitem.key}>
														<Link
															href={servicedetailitem.path}
															className="flex flex-col text-af-sm my-[16px]"
														>
															-{servicedetailitem.text}
														</Link>
													</div>
												))}
											</div>
									...
									if (item.key === 'recruit') {
										return (
											...
											<div key={item.key} className="mb-[40px]">
												...
												{recruitDetailMenu.map((recruitdetailitem) => (
													<div key={recruitdetailitem.key}>
														<Link
															href={recruitdetailitem.path}
															className="flex flex-col text-af-sm my-[16px]"
														>
															-{recruitdetailitem.text}
														</Link>
													</div>
												))}
											</div>
									...
									return (
										// Home,Company,Products,Education,News
										<div key={item.key} className="mb-[40px]">
											<Link
												href={item.path}
												className="flex items-center text-af-md"
											>
												{item.text}
												{item.key === 'education' || item.key === 'news' ? (
													<Square2StackIcon className="w-[30px] h-[30px] ml-[8px]" />
												) : null}
											</Link>
										</div>
									);
